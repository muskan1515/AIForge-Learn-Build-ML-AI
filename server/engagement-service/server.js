...
if (cluster.isPrimary) {
  ...
  (async () => {
    try {
      ...
      const server = app.listen(config.engagement_service.port, () => {
        logger.info(
          `Worker ${process.pid} running on port: ${config.engagement_service.port}`
        );
      });

      (async () => await initKafkaProducer())();

      shutdownHandler(server);

      process.on("SIGTERM", async () => await shutdownKafka());
      process.on("SIGINT", async () => await shutdownKafka());
    } catch (err) {
